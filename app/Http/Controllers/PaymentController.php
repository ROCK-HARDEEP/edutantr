...
class PaymentController extends Controller
{
    ...
    {
        if (!$identifier) {
            return $this->paymentStatusRedirect('fail');
        }
    ...
    public function enroll(string $identifier)
    {
        ...
        $decodedIdentifier = base64_decode($identifier, true);
        $transaction = TransactionRepository::query()
            ->where('identifier', $decodedIdentifier !== false ? $decodedIdentifier : $identifier)
            ->first();
        ...
        if ($transaction->is_paid) {
            return $this->paymentStatusRedirect('success', $transaction);
        }
    ...
    private function paymentStatusRedirect(string $status, $transaction = null)
    {
        ...
        return redirect("{$base}/enroll_status?status={$status}&course_id={$courseId}");
    }
}
