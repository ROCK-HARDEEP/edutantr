...
class LoginController extends Controller
{
    public function index()
    {
        ...
        if (Auth::check() && Auth::user()->email_verified_at != null) {
            ...
            if ($user->hasRole('sales_counselor')) {
                return to_route('sales.dashboard');
            }
            
            // If authenticated but no proper role, logout to prevent redirect loop
            Auth::logout();
            return redirect()->route('admin.login')->with('error', 'Your account does not have proper access permissions.');
        }
    ...
    public function authenticate(Request $request): RedirectResponse
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if ($user->is_admin && $user->hasRole('admin')) {
                ...
                return to_route('instructor.dashboard');
            } else if ($user->hasRole('sales_counselor')) {
                return to_route('sales.dashboard');
            }
    ...
    public function instructorRegister()
    {
        ...
        return view('auth.instructor.register', compact('app_setting'));
    }
