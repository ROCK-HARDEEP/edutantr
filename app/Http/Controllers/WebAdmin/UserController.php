...
class UserController extends Controller
{
    ...
    {
        $fileName = 'student-import-sample.csv';

        return response($userImportService->sampleCsv())
            ->header('Content-Type', 'text/csv')
            ->header('Content-Disposition', 'attachment; filename="' . $fileName . '"');
    }

    public function import(UserImportRequest $request, UserImportService $userImportService)
    {
        $result = $userImportService->import($request->file('file'));

        if ($result['imported'] === 0 && $result['skipped'] === 0 && !empty($result['errors'])) {
            return back()
                ->with('error', implode(' ', $result['errors']))
                ->with('import_errors', $result['errors']);
        }

        $message = __(':imported student(s) imported successfully.', ['imported' => $result['imported']]);

        if ($result['skipped'] > 0) {
            $message .= ' ' . __(':skipped row(s) skipped.', ['skipped' => $result['skipped']]);
        }
