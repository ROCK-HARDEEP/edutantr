...
class DashboardController extends Controller
{
    ...
    public function orgDashboard()
    {
        ...
        if (!$organization) {
            return redirect()->route('admin.login')->withErrors(['error' => 'Organization not found']);
        }

        $totalTransactionsAmount = TransactionRepository::query()
            ->whereHas('course', function ($query) use ($organization) {
                $query->whereHas('instructor', function ($q) use ($organization) {
        ...
        $activeCourseCount = CourseRepository::query()
            ->whereHas('instructor', function ($query) use ($organization) {
                $query->where('organization_id', $organization->id);
            })
            ->where('is_active', true)
            ->count();

        $totalEnrollmentsCount = CourseRepository::query()
            ->whereHas('instructor', function ($query) use ($organization) {
                $query->where('organization_id', $organization->id);
            })
        ...
        $totalReview = ReviewRepository::query()
            ->whereHas('course', function ($query) use ($organization) {
                $query->whereHas('instructor', function ($q) use ($organization) {
                    $q->where('organization_id', $organization->id);
                });
        ...
        $instructorCount = InstructorRepository::query()
            ->where('organization_id', $organization->id)
            ->count();

        $courseIds = CourseRepository::query()
            ->whereHas('instructor', function ($query) use ($organization) {
                $query->where('organization_id', $organization->id);
            })
            ->pluck('id')
        ...
        $studentCount = UserRepository::query()
