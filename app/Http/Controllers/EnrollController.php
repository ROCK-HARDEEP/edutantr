...
class EnrollController extends Controller
{
    ...
    public function initiateTransaction(Request $request, $courseId = null, $invoiceId = null)
    {
        ...
        $classMode = in_array($request->class_mode, ['online', 'offline']) ? $request->class_mode : 'online';
        ...
        $teamData = $this->resolveSalesTeam(request()->team_code, request()->sales_team_member_id);

        if (request()->team_code && !$teamData) {
            return $this->json('Invalid team code', null, 400);
        }
    ...
    private function resolveSalesTeam(?string $teamCode, ?int $memberId): ?array
    {
        if (!$teamCode) {
            return null;
        }

        $team = SalesTeam::where('code', $teamCode)->where('is_active', true)->first();
        ...
        if ($memberId) {
            $member = SalesTeamMember::where('id', $memberId)
