...
class CourseController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($averageRating) {
            ...
            $ceil = $floor + 0.9;
            $query->havingRaw('average_rating between ? and ?', [$floor, $ceil]);
        }
        ...
        if ($sortField === 'total_duration' && in_array($sortOrder, ['asc', 'desc'])) {
        ...
        } elseif (in_array($sortField, $sortableFields) && in_array($sortOrder, ['asc', 'desc'])) {
        ...
        $instructorFilter = function ($instructorQuery) {
            $instructorQuery->whereNull('deleted_at');
        };

        // Pagination
        $totalItems = (clone $query)->whereHas('instructor', $instructorFilter)->count();
        $perPage = $request->input('items_per_page', 15);
        $pageNumber = $request->input('page_number', 1);
        $skip = ($pageNumber - 1) * $perPage;
        $courses = $query->whereHas('instructor', $instructorFilter)
            ->skip($skip)
            ->take($perPage)
            ->get();
